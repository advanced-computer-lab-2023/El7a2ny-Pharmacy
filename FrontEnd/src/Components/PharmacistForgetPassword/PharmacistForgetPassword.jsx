...
export default function PharmacistForgetPassword() {
  ...
  async function sendData(values) {
    setIsLoading(true)
    try {
      let { data } = await axios.post(ApiBaseUrl + 'pharmacists/OTP-email', values);
      setIsLoading(false)
      navigate(`/PharmacistOtpLogin`)
    } catch (error) {
...
let validateschema = Yup.object({
  ...
  return <>
    <Helmet>
      <title>Pharmacist Forget Password</title>
    </Helmet>
    <div className="container my-5 login w-75">
      ...
      <h2 className='text-muted'>Pharmacist</h2>
      <div className="row">
